...
import { Stock } from '@/src/domain/types/Stock'
...
export const calculateMarginOfSafety = (price: number, grahamNumber: number): number => {
...
export const calculateGrahamSafetyScore = (metrics: any): number => {
  let score = 0
  const maxScore = 100
...
export const calculateGrahamValueScore = (metrics: any, price: number): number => {
  let score = 0;
  const maxScore = 100;
...
function log(message: string, data?: any) {
  console.log(`[Recalculate] ${message}`, data || '');
}

function logError(message: string, error?: any) {
  console.error(`[Recalculate Error] ${message}`, error || '');
}

async function sendUpdate(writer: WritableStreamDefaultWriter, data: any) {
  try {
    await writer.write(
      new TextEncoder().encode(`data: ${JSON.stringify(data)}\n\n`)
    );
  } catch (error) {
    logError('Failed to send update', error);
  }
}

export async function POST() {
  log('Starting market cap recalculation...');
  ...
  // Process in background while returning the stream
  (async () => {
    try {
      ...
      await sendUpdate(writer, {
        type: 'progress',
        message: 'Starting market cap recalculation...',
        processed: processedCount,
        updated: updatedCount,
        errors: errorCount
      });

      // Get all non-USD stocks that need conversion
      const { data: stocks, error: fetchError } = await supabase
        .from('stocks_v2')
        .select('*')
        .eq('is_price_usd', false);
      ...
      if (!stocks || stocks.length === 0) {
        await sendUpdate(writer, {
          type: 'complete',
          message: 'No stocks need market cap recalculation',
          processed: 0,
          updated: 0,
          errors: 0
        });
      ...
      log(`Found ${stocks.length} stocks that need market cap recalculation`);
      ...
      for (let i = 0; i < stocks.length; i += BATCH_SIZE) {
        ...
        for (const stock of batch) {
          try {
            processedCount++;
            const originalCurrency = stock.currency || 'USD';
            
            if (originalCurrency === 'USD') {
              continue; // Skip USD stocks
            }
            ...
            if (!forexResponse.ok) {
              throw new Error(`Forex API error: ${forexResponse.statusText}`);
            }

            const forexData = await forexResponse.json();
            
            if (!forexData.rate) {
              throw new Error(`No forex rate available for ${originalCurrency} to USD`);
            }

            const usdRate = forexData.rate;
            const originalMarketCap = stock.original_market_cap || stock.market_cap || 0;
            const marketCapUSD = Number((originalMarketCap * usdRate).toFixed(2));

            // Update the stock with converted market cap
            const { error: updateError } = await supabase
              .from('stocks_v2')
              .update({
            ...
            if (updateError) {
              throw new Error(`Failed to update ${stock.symbol}: ${updateError.message}`);
            }

            updatedCount++;
            log(`Updated ${stock.symbol}: ${originalMarketCap} ${originalCurrency} -> ${marketCapUSD} USD`);

          } catch (error) {
            errorCount++;
            logError(`Error processing ${stock.symbol}`, error);
          }

          // Send progress update
          await sendUpdate(writer, {
            type: 'progress',
            message: `Processed ${processedCount}/${stocks.length} stocks...`,
            processed: processedCount,
            updated: updatedCount,
            errors: errorCount
          });
      ...
      await sendUpdate(writer, {
        type: 'complete',
        message: `Market cap recalculation completed. Updated ${updatedCount} stocks, ${errorCount} errors.`,
        processed: processedCount,
        updated: updatedCount,
        errors: errorCount
      });
